...
- [ ] **M8 · Site, lot 1** 🟠 — publication par classe (S0-①) + canal d'annonces central + « ? » des apps traitées + éditeur de taxonomie minimal (créer/renommer/désactiver, garde-fou S0-⑥) + retrait de l'entrée morte « Salle d'attente Plickers » du menu admin + date du brevet ÉDITABLE (chrono 2027, B5) + AUDIT EDUSCOL terme à terme : passer les 148 libellés prof contre le guide de terminologie 2020 (premier écart trouvé le 16/07 : gram-003 « adjectif qualificatif » → « adjectif », terminologie 2020) — premier usage réel de l'éditeur, renommages sans casse (invariant id/libellé).
...
16. **MODE TEST (point de grille, confirmé 17/07)** : chaque app a un mode test aligné sur le patron QCM — génération totale en un clic (nettoyage des zombies au montage, classe _test_<app> + données complètes), test de la CHAÎNE réelle (incarnation quand l'app est interactive), purge exhaustive en sortie. À CHAQUE PASSE : si un mode test existe, l'ANALYSER (que teste-t-il réellement ? couvre-t-il la chaîne complète, y compris les mécanismes récents — injection, connexion MJPC, manifeste ?) et l'améliorer ; sinon le créer.
