...
**Confirmation acquise** : l'app découpe bien l'écran en zones (`zones-multi`, 1/2/3 colonnes selon les occupants d'une tablette), même principe que worktrack. Le code élève aura une incidence sur ce partage : chaque élève doit VOIR qu'il tape dans SON espace.
**Q1 — CODE ÉLÈVE EXIGÉ (Paul, contre le statu quo).** Raison de Paul : *« un élève qui vote est évalué sur sa capacité à évaluer. Donc pareil que toutes les autres apps. »* Le vote est archivé nominativement (`votes/<passageId>/<votant>`) et remonte à MJPC — ce nominatif était un choix délibéré. Donc portail `/codes` complet (nom + prénom + code), comme les autres apps, appliqué à CHAQUE occupant d'une zone. L'exception de la doctrine (L123, « vote anonyme → grille au panneau prof seulement ») est LEVÉE par Paul : le vote n'est pas anonyme, il est évalué.
**Q1bis — DEUX FONCTIONNALITÉS NEUVES demandées par Paul, à coder (pas seulement à consigner) :**
① **Bouton « mon voisin clique dans mon espace »** : cas vécu en réel — un élève tape dans la zone d'un autre. L'élève lésé signale l'intrusion. Conséquence sur les données : le vote/l'évaluation produit par l'intrus, s'il remonte dans MJPC, doit être **marqué d'un degré de fiabilité** (dégradé), et non silencieusement accepté.
② **Pénalité de l'intrus** : l'élève qui clique dans l'espace d'un autre se voit pénalisé. *Modalité de la pénalité À DÉFINIR avec Paul (« à voir comment »).* À cadrer avant de coder — ne pas inventer la sanction.
→ Ces deux points sont liés au fait que chaque zone est désormais attachée à une IDENTITÉ ATTESTÉE par le code : on sait qui devrait taper où, donc on peut détecter et qualifier l'intrusion.
**Q2 — MODE TEST : reprendre celui du QCM** (Paul : *« le plus capable de s'intégrer à applaudimètre, vu que le système est le même »*). `evaluation-qcm` est le patron d'origine du mode test 16bis. L'exécutant s'en inspire plutôt que des six fictifs seuls — MAIS toujours avec noms fictifs canoniques + garde sur la clé sanMJPC (le QCM lui-même a détruit 7 codes réels : reprendre sa MÉCANIQUE, pas ses noms réels en dur).
**Q3 — MOUCHARD : côté prof uniquement (Paul, après confirmation).** Vérifié : `window.onerror` GLOBAL (se déclenche sur les 3 vues — prof, tableau projeté, tablette élève), overlay `position:fixed top:0 height:60vh fond #dc2626 z-index:99999` = moitié haute de l'écran par-dessus tout, + `ErrorBoundary` React. Bug traqué : *« Cannot read 'slice' »* (appel `.slice()` sur `undefined`, non localisé). Décision : l'alerte n'apparaît QUE côté professeur, jamais sur le tableau ni la tablette ; l'erreur est journalisée pour que le bug `slice` reste traçable (voie C du cadrage).
**Q7 — OUI dans M14** : afficher les 2-3 critères aux élèves AVANT le passage (le vote par critère existe déjà, il manque l'affichage en amont). Amélioration pédagogique n°4 rattachée à M14.
**Q8 — OUI, écran « Mes lectures » ET remontée dans MJPC** (Paul : *« il faut évidemment un tel écran, et mes lectures se retrouvent aussi sur MJPC évidemment, comme mes dictées »*). Suppose le portail à code de Q1 (désormais acquis) : l'élève a une identité attestée, donc un historique consultable. À l'image de « Mes dictées » (dictee_universelle M9).
**Q5 — format corbeille : option A** (une entrée par chemin, seule compatible avec `_corbPlanRestauration` du site). **Q6 — les deux reformulations validées** : L3203 → « La séance n'est pas encore ouverte. Reste sur cet écran, elle démarre dans un instant. » ; L3211 → « La séance va commencer. Garde ta tablette prête. »
**À CORRIGER au CHANTIER** : `applause_meter` était classé « mode test conforme, à ne pas retoucher » — FAUX (purge non exhaustive, non vérifiée, état test en useState perdu au rechargement). **Au REGISTRE** : ~3 474 lignes → 3 944 ; `critereIdx` bloqué à 0 ne se reproduit pas dans les données actuelles ; 4 passages fantômes → 5.
