...
2. **CONDITIONNEL — exception assumée** : le guide en fait un temps de l'indicatif (p. 36 : « le conditionnel doit bien être considéré comme un temps de l'indicatif » — tout en notant : « dans l'usage scolaire, le conditionnel est parfois considéré comme un mode et enseigné comme tel »). Décision : on ACCEPTE AUSSI « mode conditionnel », avec son PRÉSENT et son PASSÉ. Conséquence : la famille fam-25 doit couvrir le conditionnel passé (item à créer, voir liste des manquants) ; « mode conditionnel » entre dans les alias acceptés.
3. **gram-003 — exception assumée, INVERSE de la proposition E1** : on MAINTIENT le libellé « Identifier un adjectif qualificatif » et on ACCEPTE « adjectif » (alias). Aucun renommage.
4. **TYPES DE PHRASE — ALIGNEMENT (option a de E2)** : trois types (déclarative, interrogative, impérative — p. 25 : « le type impératif correspondant à l'injonction ») ; l'exclamative REJOINT LES FORMES (p. 26 : « L'exclamation n'est pas un type d'acte spécifique […] classée parmi les formes de phrases »). Conséquences : renommage de gram-018 + création d'une notion « forme exclamative » dans fam-15 (voir liste des manquants).

## DÉCLARATION DE COUVERTURE
**COUVERTURE INTÉGRALE ATTEINTE** : Paul a téléversé le PDF officiel ; extraction locale complète (`pdftotext -layout`, 213 pages, 542 402 caractères, fin vérifiée au colophon — ISBN 978-2-11-155900-4, réimpression janvier 2023). Le texte intégral est conservé dans le conteneur (`guide-eduscol-integral.txt`) et toutes les vérifications ci-dessous citent leurs pages.

**LES TROIS POINTS [À CONFIRMER] SONT CLOS :**
- **C1 · CLOS, CONFORME** — « adjectif verbal » est le terme du guide (p. 153, II.3.9.5.2) : degré d'intégration maximal du participe présent à la catégorie de l'adjectif. `conj-024` conforme.
- **C2 · CLOS, HORS PÉRIMÈTRE DU GUIDE** — le discours rapporté (direct / indirect / indirect libre) est ABSENT du guide : zéro occurrence sur les 213 pages. `gram-029` à `032` ne peuvent pas être en écart avec lui ; leur vocabulaire relève des programmes du cycle 4, où il figure tel quel. Aucun changement.
- **C3 · CLOS, CONFORME** — le guide nomme « fonction de connecteur » (fonction textuelle, II.2.7), « connecteurs argumentatifs » et « connecteurs textuels » (II.1.6). Les libellés fam-18 classés par sens (cause, conséquence, but…) sont descriptifs et compatibles. Aucun changement.

**Vérifications complémentaires sur le texte intégral** : le débat « temps ou mode ? » du conditionnel est documenté par le guide lui-même (encadré p. 146-148) avec la tolérance scolaire explicite — l'exception de Paul est pleinement défendable ; « voix active/passive » (p. 141) qualifie le VERBE tandis que « forme passive » qualifie la PHRASE — gram-020 (formes de phrase) est du bon côté ; l'attribut du COD est confirmé (p. 89, II.2.3.2.2) ; le gérondif est « l'emploi adverbial du participe présent » (p. 153-154).
## VERDICT D'ENSEMBLE
