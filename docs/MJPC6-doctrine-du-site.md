...
**LE POINT DUR DU CLONAGE — la propagation. RECTIFICATION DU 21/07 : ce n'est PAS une dérive, c'est une FILE D'ATTENTE.** Vérification faite : le socle `mjpc-core.js` v1.1.0 (14/07) n'apporte qu'une chose, son §8 — la **session partagée** (`lireSessionMJPC`, `validerEleveMJPC`), c'est-à-dire le BYPASS. Donc « 1.0.0 » ne signifie pas « app en retard et à risque » mais **« app n'ayant pas encore reçu le bypass »** — ce qui est précisément l'objet des morceaux M9→M13, et ce qui explique les constats de Paul du 20/07 (`reecriture_bb4e` demandant un nom sans code, `worktrack` affichant « Choisis ta classe »). **Contrôle du point qui aurait été réellement inquiétant** : le socle a-t-il été modifié LOCALEMENT quelque part ? Empreintes comparées dans six apps — `sanMJPC` identique partout, `extractEleves` identique partout, `lireSessionMJPC` identique dans les trois apps qui le portent. **Le bloc est resté un bloc ; personne ne l'a bricolé.** État du déploiement :
...
**Ce qui reste vrai pour le gabarit** : cloné à son tour, il connaîtra la même file d'attente — et il faudra pouvoir répondre à « quelles apps portent la version courante ? » sans ouvrir onze fichiers.
...
### SITE-COURS · L'ATELIER DE COMPOSITION — DÉCISIONS PRISES EN DISCUSSION (27/07, Paul × conscience n°3)
Ces décisions COMBLENT les manques de la doctrine du site §XIII avant passation. À transmettre à la conscience suivante qui cadrera le chantier SITE-COURS.
**① CASES GRISÉES : GUIDER SANS JAMAIS VERROUILLER.** Le gabarit grise les combinaisons incohérentes, MAIS un clic sur une case grisée L'ACTIVE quand même, avec un avertissement invitant à regarder l'aperçu — *« parce qu'en réalité c'est lui qui fait foi »* (Paul). Principe : la machine conseille, ne décide jamais. (Rejoint « le prof a tous les droits, jamais bloqué ».) Les dépendances déclarées servent le grisage ET rendent les préréglages quasi gratuits (un produit = un jeu de cases cohérent, Paul pourra en créer sans livraison).
**② LE MODÈLE MENTAL, dans les mots de Paul** : *« J'ai ma feuille vierge, et au fur et à mesure que je coche des cases, des choses apparaissent sur la feuille. Ça nécessite aussi que le contenu de la feuille soit éditable via des champs, exactement comme dans worktrack. »* → UN SEUL gabarit (la feuille) = les CASES de `correction_dictee` (afficher/masquer, 4 cases en langage clair : « Afficher la note sur la copie », etc.) + les CHAMPS ÉDITÉS EN DIRECT de worktrack (`setMeta` : le contenu se met à jour à l'écran immédiatement). Deux mécanismes éprouvés à assembler, rien à inventer. PAS de jargon (« nature », « destination », « composante ») dans l'interface : des phrases en français.
**③ LES PRODUITS = des boutons qui pré-cochent des lots de cases** (pistons d'orgue), PAS des gabarits séparés. La destination (papier / écran / projection) est UNE CASE ORDINAIRE dans le lot (« Pour impression : pointillés + date d'édition »), pas une dimension à part. Liste des produits arrêtée avec Paul, collée aux rubriques EXISTANTES du site (onglet transversales : sections Langue / Brevet / Aller plus loin, vérifiées dans le code — compteurs `count-fiches-grammaire`, `count-fiches-notions`, `count-prepa-brevet`, `count-autonomie`, `count-analyse-logique`) :
  1. Fiche de séance (chapitre, cahier) — LE produit prioritaire pour la saisie d'été
  2. Support de cours (projection)
  3. Bilan (retour élève)
  4. Fiche grammaire → section Langue
  5. Fiche notion → section Langue [NB : « fiches notions » de l'onglet transversales, PAS l'éditeur de taxonomie M8bis — homonymie levée]
  6. Prépa brevet → section Brevet
  7. Fiche analyse logique → section Aller plus loin
  8. Point d'étape (agrégatif — dépend de M15 + Concordance, place réservée, codé plus tard)
**④ STOCKAGE : VOIE 2 — BIBLIOTHÈQUE DE DOCUMENTS COMPOSÉS.** La feuille composée est un DOCUMENT RÉUTILISABLE vivant dans une bibliothèque à part ; le bouton « LIER » existant du site (modale `linkModalApplyUrl` / `linkModalApplyApp` / `linkModalUnlink`, vérifiée dans le code) gagne une TROISIÈME source : un document d'atelier. Un document, PLUSIEURS points d'affichage (une fiche passé simple liée en section Langue ET dans une séance de 4e ET de 3e, sans duplication ; corrigée une fois, corrigée partout).
**⑤ LE SPIRALAIRE SUR LA FICHE UNIQUE (Paul, 27/07)** : *« ça permet de faire vivre sur une fiche la totalité du spiralaire : si la section 6e est affichée, alors elle apparaît en 5e, en 4e, en 3e. On rejoint mon principe du 3e qui a accès aux niveaux inférieurs. Et chacun sait par quoi il est concerné : quand l'app détecte que c'est un sixième qui se connecte, il voit son document avec uniquement ce qui le concerne sur une notion. »* → Une fiche de notion porte des STRATES PAR NIVEAU ; l'affichage filtre selon le niveau du connecté (le 6e voit sa strate ; les niveaux supérieurs voient la leur + les inférieures — principe des niveaux en série de la doctrine §II). **QUESTION TRANCHÉE PAR PAUL LE 29/07 : L'IMPRESSION EST AU CHOIX** (*« au choix évidemment »*, sur les trois options instruites par la conscience n°4). Une fiche spiralaire s'imprime avec un réglage « jusqu'où on remonte », dont le **défaut est la strate du niveau visé + les strates inférieures** — c'est-à-dire exactement ce que l'élève voit à l'écran. Réductible à la seule strate courante, extensible à toutes. **Trois raisons du défaut retenu** : ① il ne décide pas à la place du professeur au moment où il imprime (« du proposé, jamais de l'imposé ») ; ② il fait coïncider le papier et l'écran, donc l'élève colle au cahier ce qu'il a consulté en ligne ; ③ il ne coûte presque rien, le filtrage par niveau existant déjà pour l'écran — l'impression réutilise le même filtre avec un curseur au lieu d'une valeur figée. **Le réglage vit dans le LOT DU PRODUIT, pas à l'écran d'impression** : c'est une case ordinaire, pré-réglée par le bouton de produit (une fiche de séance tire « ma strate seule », un point d'étape tire « tout »), pour ne pas encombrer une interface qui doit rester épurée.
**Acquis relus dans la doctrine §XIII (inchangés)** : atelier ≠ visionneuse de fichiers externes, mais l'APERÇU navigable est composante de premier rang · gabarits en dur, contenus en base · le composant se CLONE (jamais centralisé) avec version publiée au manifeste · images sur Drive lien public, redimensionnées à l'envoi · diapositives via prompt IA → JSON (jamais HTML), relecture obligatoire · liste exhaustive des composantes familles A-K · cahier = profil imprimé, date d'édition obligatoire, agrégation À LA DATE · fenêtre glissante PAR TYPE · taxonomie = couture, taggage par notion = PRÉREQUIS des composantes agrégatives · prototype complet = `correction_dictee` (cases + aperçu navigable `previewIdx` + sortie).

### SITE-COURS · L'ÉDITEUR VIVANT ET LA BANQUE DE CONSIGNES (discussion Paul × conscience n°4, 29/07)
**Le renversement, dans les mots de Paul** : *« on pourrait faire de cet éditeur un vrai éditeur : avec insertion de ligne pointillée supplémentaire (un bouton + ou − qui insère en direct des lignes dans la visionneuse), insertions de consignes éditables (et enregistrables, afin de pouvoir les cliquer pour les fois ultérieures, regarde comment fonctionne la correction dans réécriture). »*
**Ce que cela change** : l'aperçu cesse d'être un miroir, il devient **la surface d'édition**. On n'agit plus sur un panneau de réglages qui se répercute ; on agit sur la feuille elle-même. Conséquence sur le modèle : une composante n'est plus seulement cochée ou non — certaines portent un **PARAMÈTRE RÉGLABLE SUR PLACE** (nombre de lignes pointillées, hauteur d'une zone vierge, nombre de colonnes d'un tableau), et le réglage se fait à l'endroit où l'on en voit l'effet.
**LE PATRON DÉSIGNÉ PAR PAUL — la correction de `reecriture.html`, lue sur pièces** : le texte attendu devient une surface CLIQUABLE mot par mot ; un clic ouvre une saisie (« Que tapent les élèves au lieu de X ? ») ; la saisie devient un piège `{tokenIdx, attendu, fautif}` ; **le mot porte ensuite sa mémoire visuellement** (fond ambré, soulignement, gras) avec une info-bulle listant les pièges déjà connus et invitant à en ajouter ; les pièges s'affichent en puces retirables ; **un même mot en accumule plusieurs, on n'écrase jamais**. La valeur du mécanisme n'est pas dans le clic : elle est qu'on **dépose une observation au moment où on la fait**, et que la liste se constitue par sédimentation puis revient déjà remplie la fois suivante. C'est « capitalisation et cercle vertueux » (Paul, 21/07) rendu concret.
**LA TRANSPOSITION, ET SA LIMITE À NE PAS FRANCHIR** : on capitalise ici des **CONSIGNES ENTIÈRES**, pas des mots à l'intérieur d'un texte. **Aucune tokenisation n'est donc nécessaire, et aucune ne doit être écrite** — la règle absolue (point 27) interdit la dix-septième. Cloner le patron trop fidèlement, mot à mot, serait précisément le piège.
**PRINCIPE GRAVÉ (Paul, 29/07) — RIEN NE DOIT NE PAS ÊTRE MODIFIABLE.** *« Elles doivent toujours rester éditables. Rien ne doit ne pas être modifiable. »* Une consigne capitalisée n'est jamais figée : elle se reprend, se corrige, se remplace. Le principe dépasse les consignes et rejoint le point 26 (tout texte élève éditable) en l'étendant à TOUT contenu produit par le professeur. **La seule exception, et elle n'en est pas une** : ce qui IDENTIFIE ne se modifie pas — les identifiants opaques restent immuables pendant que leurs libellés changent (règle déjà posée pour la taxonomie : « on ne déchire jamais une page du référentiel, on la range »). Tout ce qui S'AFFICHE est modifiable ; ce qui sert à retrouver l'historique ne l'est pas, sinon l'historique se casse.
**DÉCISION DE CIRCUIT (Paul, 29/07) : ON N'INTERROMPT PAS L'EXÉCUTANT EN COURS.** Le paramètre réglable et la banque de consignes ne sont donc PAS versés à SITE-COURS-1, en vol au moment de la discussion. *Estimation rectifiée par la conscience n°4 : elle avait annoncé qu'attendre imposerait « de refaire ① » — c'était exagéré. Un schéma déclaratif est fait pour absorber un champ nouveau (« pour ajouter un type ou un champ, je touche un seul endroit ») ; le surcoût réel porte sur l'INTERFACE — rendre interactif un aperçu conçu comme passif — et sur le rendu paramétré, qui sont un travail neuf de toute façon. Avoir failli faire interrompre un exécutant sur un pronostic gonflé est le travers « catastropher », consigné comme tel.*
**QUESTION OUVERTE, la plus structurante — MODIFIER UNE CONSIGNE MODIFIE-T-ELLE LES DOCUMENTS PASSÉS ?** Deux principes de la doctrine se rencontrent ici : « un document, plusieurs points d'affichage — corrigée une fois, corrigée partout » (§XIII décisions ④) et « le papier est un instantané assumé, l'agrégation est À LA DATE » (§XIII.6). Ils ne se contredisent pas — à l'écran le document est vivant, sur le papier il est figé au jour de son édition, et la date d'édition le dit — mais le cas des consignes doit être tranché explicitement. *Proposition de la conscience, non validée : le document embarque une COPIE de la consigne au moment où il l'utilise, plus un lien de provenance vers la banque ; si la consigne change ensuite, le document PROPOSE la mise à jour sans jamais l'imposer (« rien de bloquant ni d'automatique pour le prof : que du proposé »). Sans quoi une modification de janvier changerait le sens d'une fiche de septembre déjà collée dans les cahiers, et le cahier ne correspondrait plus au site.*
**TROIS QUESTIONS SUBORDONNÉES, à trancher au cadrage de SITE-COURS-2** : ① une consigne capitalisée porte-t-elle sa NOTION (ce qui en ferait un gisement pour la Concordance et l'Atelier, au lieu d'un presse-papier amélioré) ? ② la banque est-elle globale ou rangée PAR TYPE de séance (les types existent déjà dans les données : `etude_texte`, `dictee_reecriture`, `remediation`…) ? ③ une consigne modifiée écrase-t-elle l'ancienne ou s'ajoute-t-elle (réécriture a tranché dans son coin : les pièges s'accumulent, rien ne s'écrase) ?
**PLACEMENT** : la banque de consignes ÉLARGIT L'OBJET de **SITE-COURS-2**, qui devient *ce qu'on écrit une fois et qu'on réutilise* — documents entiers ET fragments. Le paramètre réglable sur l'aperçu y rejoint sa place.

### SITE-COURS · LE PORTE-VUE, LES DEUX FAMILLES DE DOCUMENTS, ET LA MISE À JOUR COMME ÉVÉNEMENT PÉDAGOGIQUE (Paul × conscience n°4, 29/07)
**RECTIFICATION D'ABORD** : la conscience n°4 avait cru trouver que « corrigée une fois, corrigée partout » (décision ④ du 27/07) était une suggestion de la conscience n°3 consignée à tort comme validée. **C'ÉTAIT FAUX, et Paul l'a corrigée** : la décision était bonne — *« l'histoire de la voie 2 c'était correct »* — mais elle portait sur **LES FICHES**, pas sur les documents de séance. L'erreur était une GÉNÉRALISATION : appliquer à tous les documents une décision prise pour une catégorie. Leçon : avant de dénoncer une décision comme non validée, vérifier son PÉRIMÈTRE — une décision juste appliquée trop largement produit exactement les mêmes symptômes qu'une décision inventée.
**LA LIGNE DE PARTAGE, dans les mots de Paul** : *« Une correction propagée sur des consignes, c'est con. Une correction propagée sur du passé simple, c'est rentable à une échelle immense. »* Elle ne passe donc PAS entre l'écran et le papier (hypothèse abandonnée de la conscience n°4) mais entre **CE QUI EST ANCRÉ DANS UN MOMENT** et **CE QUI EST RATTACHÉ À UNE NOTION**. C'est la distinction `ressource`/`acte` du §II appliquée aux documents composés.
- **ANCRÉS** — fiche de séance, support de cours, bilan, point d'étape : un seul point d'affichage, aucune propagation, vocation à être conservés.
- **TRANSVERSAUX** — fiche grammaire, fiche notion, prépa brevet, fiche analyse logique : plusieurs points d'affichage, propagation VOULUE (sans elle, sept copies divergentes de la même fiche), vivants par nature, ils s'améliorent d'année en année.
**LA TYPOLOGIE EXISTE DÉJÀ DANS L'ARCHITECTURE DU SITE — vérifié dans `index.html`** : les quatre transversales atterrissent exactement dans les sections de l'onglet transversales (Langue → `count-fiches-grammaire`, `count-fiches-notions` ; Brevet → `count-prepa-brevet` ; Aller plus loin → `count-analyse-logique`), les quatre ancrées dans les séances des chapitres. **Le lieu d'atterrissage dit déjà le comportement** : rien à inventer, seulement à nommer.
**LE PORTE-VUE (apport de Paul, 29/07) — DEUX CONTENANTS PHYSIQUES, PAS UN** : *« un élève ne colle jamais une fiche notion ou une fiche grammaire, un document non ancré. Celui-ci va dans un porte-vue spécifique dédié à ces fiches-là. »* La typologie n'est donc pas une abstraction de conception : **elle correspond à deux objets dans le cartable.** Le CAHIER est chronologique et sédimentaire — on y colle, on n'en retire jamais rien. Le PORTE-VUE est rangé par notion et, propriété décisive, **on peut y REMPLACER une feuille**.
**CONSÉQUENCE DIRECTE SUR LE GABARIT — une case de la famille J qui en commande plusieurs autres** : « pour le cahier » (demi-A4 ou A4 réduit, pointillés de découpe utiles, recto seul puisqu'une face est collée) contre « pour le porte-vue » (A4 plein au format de la pochette, aucune découpe, recto-verso possible). Le bouton de produit la tire d'office : une fiche de séance sait qu'elle va au cahier, une fiche de notion au porte-vue. Rien à demander à l'utilisateur, tout à déclarer.
**LA MISE À JOUR D'UNE FICHE DEVIENT UN ÉVÉNEMENT PÉDAGOGIQUE (apport de Paul)** : *« une correction apportée sur le site peut faire partie des notifications à l'élève (le professeur a mis à jour telle partie de tel document, fais la modification en recopiant ce que tu vois à l'écran) et il aurait une vue zoomée sur la partie à corriger, avec éventuellement un petit rituel de propreté. »* La propagation cesse d'être une commodité technique qui économise le travail du professeur : **elle fait travailler l'élève**, la recopie d'une définition corrigée étant de la récupération active. Ce que le mécanisme suppose : ① les fiches transversales sont **VERSIONNÉES** — sans quoi on ne peut pas dire ce qui a changé, et cela donne son sens plein au bouton « conserver », qui produit le point de comparaison au lieu d'archiver un déchet ; ② le canal existe déjà (`site/annonces`, en production depuis M8, lu par le « ? » des apps) mais il s'agit d'une annonce d'un genre nouveau, **ciblée sur un document** ; ③ la vue zoomée est un affichage de DIFFÉRENCE entre deux versions — montrer ce qui a changé sans exiger de l'élève qu'il compare deux pages.
**TROIS POINTS TRANCHÉS PAR PAUL LE 29/07** :
**(a) SEUIL DE NOTIFICATION — validé** : toute correction ne s'annonce pas. Le geste de mise à jour laisse choisir entre correction discrète et annonce aux élèves. Jamais d'automatique : une notification systématique deviendrait du bruit et plus personne ne la lirait.
**(b) FORMULATION — validée à l'accompli, première personne** : « J'ai précisé la définition du passé simple — reprends-la sur ta fiche ». La formulation spontanée « le professeur a mis à jour… » butait sur le point 28 (le professeur ne se désigne jamais à la troisième personne) ; « j'ai corrigé une erreur » est à proscrire (s'attribuer la faute au lieu de décrire le flux). Textes définitifs SOUMIS.
**(c) L'APP DÉTECTE SI LA RECOPIE EST RAISONNABLE — exigence de Paul** : *« l'app doit détecter si les modifications sont trop profondes pour que la recopie ne coûte pas trop en temps et en ingénierie élève (colle, ciseaux, recopiages qui débordent, présentation mal gérée etc). »* Le verdict est PROPOSÉ, Paul garde la main.

### SITE-COURS · LA MISE À JOUR D'UNE FICHE — VERSION SIMPLE RETENUE, ET LA PISTE ABANDONNÉE AVEC SA RAISON (29/07)
**Verdict de Paul sur la première proposition** : *« non en fait c'est un flux compliqué. Propose-moi quelque chose de simple avec l'existant et le prévu. »*
**CE QUI A ÉTÉ ABANDONNÉ, et POURQUOI — à ne pas réinventer** : la conscience n°4 avait proposé un dispositif à trois régimes (recopie / vignette imprimée à coller / réimpression), avec **détection automatique de la profondeur d'une correction** — cas bloquants (pagination qui change, zone ajoutée ou supprimée, tableau restructuré), mesure du coût en **surface de papier à recouvrir** plutôt qu'en caractères, facteurs aggravants (nombre de zones touchées, insertion qui décale le texte suivant), et planche de vignettes calibrées imprimée par classe. **Motif de l'abandon : sur-conception.** Le dispositif prétendait calculer ce que Paul sait déjà — il vient de faire la correction, il connaît sa portée, et le dire lui coûte une demi-phrase dans la même respiration. **Ces raffinements restent possibles PLUS TARD, posés sur le socle simple sans le défaire, mais ils ne se conçoivent bien qu'après une année d'usage réel** : aujourd'hui ce serait deviner ce qui aura manqué.
**LA VERSION RETENUE — elle tient dans ce qui existe déjà** :
① **LA DATE D'ÉDITION FAIT L'ESSENTIEL DU TRAVAIL.** Déjà obligatoire, déjà imprimée sur tout document (§XIII.6). La fiche du porte-vue porte « 12 septembre », celle de l'écran porte « 8 janvier » : **l'élève voit que sa feuille est périmée sans qu'aucun mécanisme ne le lui calcule.** Ni versionnement, ni différentiel, ni vue zoomée.
② **UNE CASE ET UN CHAMP.** À l'enregistrement d'une fiche transversale, une case « prévenir les élèves » ouvre un champ de texte libre. Ce que Paul y écrit part dans le **canal d'annonces existant** — `site/annonces`, format `{app, date, texte}`, en production depuis M8, filtre `!a.app || a.app === "<id>" || a.app === "*"`, tri par date décroissante, 8 affichées, écriture par `ecrireAnnonce(id, texte, app)` avec clé anti-collision. **Rien à construire, seulement à brancher.**
③ **C'EST PAUL QUI DIT LA PORTÉE**, pas l'app : « J'ai précisé la définition du passé simple — reprends-la sur ta feuille » ou « La fiche sur les compléments a beaucoup changé, je te la réimprimerai ». La machine ne conseille même pas : **elle transporte**.
**CE QUE LA SIMPLIFICATION RÈGLE AU PASSAGE** : la question des destinataires (« à tous ceux qui ont un jour reçu la fiche, ou aux seules classes en cours de chapitre ? ») **disparaît** — l'annonce est ciblée par app dans le format existant et par niveau puisque le site se parcourt niveau par niveau. Aucun suivi de qui détient quelle fiche n'est nécessaire.
...
### SITE-COURS · REPRISE DE LA CONCEPTION DE L'ÉDITEUR APRÈS PROMOTION DE SITE-COURS-1 (Paul × conscience n°4, 29/07)
**Trois apports de Paul après avoir vu l'atelier tourner** : ① beaucoup de cases gagneraient à devenir des **curseurs à crans** ; ② le **précochage** doit être plus présent qu'il ne l'est ; ③ il faut porter dans l'atelier **ce qui fait l'efficacité de l'éditeur worktrack : la zone prompt IA → JSON** — *« injection rapide dans le gabarit du json issu d'une discussion IA cadrée, et ensuite, réglages fins par moi. Je parle ici autant de structure de documents que de contenus. »*
**LE PATRON WORKTRACK, LU SUR PIÈCES** (`PROMPT_CHAPTER`, `promptView`, `injectView`, `doInject`) : un **prompt template ÉDITABLE** qui impose d'abord une discussion de cadrage (« NE PRODUIS AUCUN JSON TOUT DE SUITE… Procède par allers-retours. Reformule, propose, mais attends mes validations ») et ne libère le JSON **que** sur l'ordre explicite « produis le JSON » ; un bouton **Copier le prompt** ; une **zone de collage** ; puis `JSON.parse` → **`validateChapter()`** → refus avec **message nommé** ou chargement. Plus une alerte de diagnostic qui ramène à l'injection quand le chapitre canonique manque.
**ORDRE DE TRAVAIL ÉTABLI** : la zone prompt IA passe AVANT les curseurs et le précochage. Les curseurs changent l'interface d'un éditeur qui existe ; **la zone prompt change le MODE D'ENTRÉE** — on ne part plus d'une feuille vierge qu'on garnit, mais d'un jet qu'on affine — et elle détermine donc ce que les cases ont encore à faire.
**Q1 — LANGAGE DU JSON : LES IDENTIFIANTS DE L'ATELIER** (décision de Paul, *contre* le conseil de la conscience qui proposait des blocs nommés en français). **Ce que la décision rend possible et que le conseil manquait** : les 121 identifiants et leurs libellés vivent DÉJÀ dans `ATELIER_COMPOSANTES` — **la liste remise à l'IA se GÉNÈRE donc depuis le schéma** au lieu d'être écrite à la main, et reste juste automatiquement quand une composante est ajoutée. Aucune table de correspondance à maintenir. **Contrepartie à traiter** : une IA peut inventer un identifiant, d'où une **validation à messages nommés** sur le patron de `validateChapter` — un identifiant inconnu se refuse en le désignant, jamais en silence. Le prompt étant **un par produit**, il ne présente que les identifiants pertinents, ce qui borne sa longueur.
**Q2 — L'INJECTION NE REMPLACE PAS : ELLE ENTRE DANS UN CLASSEUR** (apport de Paul, plus ambitieux que la proposition de la conscience). *« Le bouton nouvelle feuille doit pouvoir créer une sorte de classeur. Quand je clique sur nouvelle feuille, je vois partir l'ancienne sur la droite, un peu réduite (ou en carte à cliquer, à voir), et je peux ainsi travailler sur plusieurs feuilles en même temps, et l'injection affichée correspond à la feuille affichée. »* L'atelier cesse d'être un éditeur de document unique pour devenir un **espace de travail à plusieurs feuilles simultanées** ; l'injection et toutes les commandes portent sur la **feuille au premier plan**.
**Q3 — UN PROMPT PAR PRODUIT**, tous partant du même squelette de cadrage.
**Q4 — LE PROMPT VIT EN FIREBASE** dès le départ (celui de worktrack n'est mémorisé que sur le poste, dette déclarée dans son code) : sans quoi les prompts affinés ne suivent pas Paul du bureau au téléphone.
**QUESTIONS OUVERTES SUR LE CLASSEUR, à trancher avant de coder** : son nom (« classeur » entre en collision avec le vocabulaire physique déjà chargé — le CAHIER reçoit les documents ancrés, le PORTE-VUE les transversaux) · sa persistance (session de travail éphémère, ou objet durable à créer/renommer/supprimer) · sa disposition à 390 px, où une bande de cartes latérale est impossible · son articulation avec l'écran « Mes feuilles » déjà livré et avec la bibliothèque de SITE-COURS-2.
